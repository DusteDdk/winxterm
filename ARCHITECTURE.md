...
`src/winxterm_host.c` owns bidirectional ConPTY hosting for console clients. It creates the pseudoconsole, launches the direct child process with `STARTUPINFOEXW`, records host lifecycle state and child information in the bridge, reads ConPTY output into the parser while visible, drains and discards output while running headless after a keep-running close, writes queued keyboard input to ConPTY input, applies pending `ResizePseudoConsole` requests, and cleans up process, pipe, and pseudoconsole handles.
...
When the window is closed while a direct ConPTY child is running, winxterm shows a native confirmation dialog listing that direct child with `Keep running` checked by default. Accepting the checked default destroys the window but keeps the winxterm process alive headless until the direct child exits; output is drained and discarded with byte/line summaries written to the debug log only if logging is enabled. Unchecking `Keep running` requests bounded child shutdown and terminates the direct child if it does not exit promptly. The dialog tracks only the direct ConPTY child launched by winxterm; descendant processes are a known limitation of the current Windows-native host policy.
...
Completed `dstshell` command lines are split into pipeline groups separated by `;` and `&&`; only adjacent `|` segments form a streaming pipeline. A standalone external command inherits `dstshell`'s ConPTY-backed stdin, stdout, and stderr as a normal child process, which is the required mode for full-screen terminal clients such as `vim`. A command that participates in a pipeline uses explicit stdio handles instead: upstream and downstream pipeline edges are kernel pipes, first-stage terminal stdin is pumped from shell stdin, last-stage stdout and every stderr stream are pumped back through shell stdout, and `CREATE_NO_WINDOW` prevents hidden helper console windows. Pipeline stages are started before waiting for completion so data streams through pipes instead of being buffered stage-by-stage in memory. Pipeline exit status is the final stage status, and `&&` uses that status. Builtins in a pipeline run in isolated stage shells and route stdout through the same planned stream endpoints, so parent-shell state such as cwd does not leak from pipeline stages.
